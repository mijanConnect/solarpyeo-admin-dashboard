...
import TechnicalSupportDetailModal from "./IdentityDisputeDetailModal";
import {
  useGetIdentityDisputeSubmissionsQuery,
  useUpdateIdentityDisputeSubmissionMutation,
} from "../../../redux/apiSlices/identityDisputeSubmission";

const { Option } = Select;

const IdentityDisputeSubmission = () => {
  // local UI state
  ...
  if (submissionType && submissionType !== "All") {
    ...
    const apiStatus = statusMap[submissionType] || submissionType.toLowerCase();
    queryParams.push({ name: "progressStatus", value: apiStatus });
  }
  ...
  const {
    ...
    error,
    refetch,
  } = useGetIdentityDisputeSubmissionsQuery(queryParams);
  ...
  const tableData = useMemo(() => {
    ...
    return items.map((item, index) => {
      ...
      const machineStatus = (item.progressStatus || "").toString();
      ...
      const humanize = (s) =>
        (s || "")
          ...
          .join(" ");
      let displayStatus;
      if (machineStatus.toLowerCase() === "review") {
  ...
  const handleAcceptSubmit = async () => {
    ...
    try {
      await updateSubmission({
        id: selectedRecord._id,
        body: { progressStatus: "review" },
      }).unwrap();
      setIsAcceptModalVisible(false);
      message.success("Status updated to review!");
    } catch (err) {
      console.error(err);
  ...
  const directAccept = (record, newStatus = "review") => {
    Modal.confirm({
      title: "Are you sure?",
      content: "Do you want to update this submission status?",
      okText: "Yes, Update",
      cancelText: "Cancel",
      async onOk() {
        try {
    ...
    Modal.confirm({
      ...
      async onOk() {
        try {
          ...
          await updateSubmission({
            id,
            body: { progressStatus: "pending" },
          }).unwrap();
          message.success("Submission status updated!");
          refetch();
        } catch (err) {
          console.error(err);
    ...
    <div className="">
      ...
      <div className="flex justify-between items-end bg-red-300 p-3 rounded-lg mb-4 mt-4">
        <p className="text-[25px] font-semibold ml-1">
          User Identity Dispute / Mistaken Identity Claim Submissions
        </p>
...
export default IdentityDisputeSubmission;
